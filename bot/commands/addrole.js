...
if(!user) return message.reply("❌ | Voce precisa mencionar um usuario").then(msg => msg.delete({timeout: 5000}))
if(!role) return message.reply("❌  | Voce precisa mencionar um cargo").then(msg => msg.delete({timeout: 5000}))

if(message.guild.members.cache.get(user.id).roles.cache.has(role.id)) return message.reply("Cargo?")
